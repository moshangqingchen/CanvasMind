...
export async function GET(request: Request) {
  ...
  const catalog = await loadCyberAfeiCatalog({
    force: refresh,
    fetch: providerFetch,
  });
  const connections = (await getRepository().listConnections()).filter(
    (connection) => connection.config.preset === CYBERAFEI_PRESET_ID,
  );
  ...
  if (group) {
    ...
    const scan = await scanCyberAfeiConnection(connection.id, {
  ...
  // The settings overview should be a fast, read-only catalog lookup. A full
  // keyed inventory scan is reserved for an explicit refresh request (or the
  // group-specific view above).
  if (!refresh)
    return NextResponse.json({
      checkedAt: catalog.checkedAt,
      source: catalog.source,
  ...
  const scans = await Promise.all(
    [...connectionByGroup.values()].map((connection) =>
      scanCyberAfeiConnection(connection.id, {
        forcePricing: true,
        persist: true,
      }),
    ),
